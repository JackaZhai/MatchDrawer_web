...
class AuthManagementAssetsTest(unittest.TestCase):
    ...
    def test_settings_page_contains_account_security_and_admin_management_sections(self):
        ...
        api_page = html.split('id="page-api-keys"', 1)[1].split('id="page-comfyui-workbench"', 1)[0]
        self.assertIn('<div class="keys-status">', api_page)
        self.assertIn("<h3>密钥管理</h3>", api_page)
        self.assertIn("用量按账号记录", html)
        self.assertNotIn('class="api-global-note"', api_page)
        self.assertNotIn("此处配置平台全局 API", api_page)
        self.assertNotIn('<h3 class="content-card-title">全局 API 设置</h3>', api_page)
